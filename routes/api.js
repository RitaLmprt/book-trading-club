...
router.put('/return/:id', function(request, response) {
    console.log('in reject route');
    console.log(request.body.borrower);
                            ...
                            User.findById(borrower, function(err, user){
                                ...
                                user.save(function(err, res){
                                    if(err) {
                                     ...
                                     //return response.status(204).send('Trade went through. status should be 0 and rejected');
...
router.put('/reject/:id', function(request, response) {
    console.log('in reject route');
    console.log(request.body.borrower);
    console.log(request.body.owner);
    console.log(request.params.id);
...
router.put('/borrow/:id', function(request, response) {
    ...
    if(borrower === owner){
        return response.status(400).send('Can not borrow your own book');
    }
    Books.findOne({ _id: bookId }, function(err, book) {
        if(err) {
            return response.status(400).send(err);
...
//Search ingle books API to retrieve covers and title
